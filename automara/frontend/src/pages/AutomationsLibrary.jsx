...
function AutomationsLibrary() {
  ...
  // Extract required configuration fields from workflow
  const extractRequiredFields = (workflow) => {
    const fields = [];
    ...
    try {
      ...
      console.log('Extracting fields from workflow data:', data);
        ...
        data.nodes.forEach(node => {
          ...
          */
          ...
          if (nodeType.includes('postgres') || nodeType.includes('mysql') || nodeType.includes('database')) {
            ...
            fields.push({
              key: `${node.id}_db_name`,
              label: `${node.name} - Database Name`,
              type: 'text',
              nodeId: node.id,
              nodeName: node.name,
              nodeType: 'database',
            ...
            fields.push({
              key: `${node.id}_db_user`,
              label: `${node.name} - Database User`,
              type: 'text',
              nodeId: node.id,
              nodeName: node.name,
              nodeType: 'database',
  ...
  // Load saved configuration for workflow
  const loadWorkflowConfig = (workflowId) => {
    const savedConfigs = JSON.parse(localStorage.getItem('workflow_configs') || '{}');
    return savedConfigs[workflowId] || {};
  };
  ...
  // Open settings modal
  const handleOpenSettings = (e, workflow) => {
    e.stopPropagation();
      ...
      <div className="max-w-7xl mx-auto">
                  ...
                  <div className="mt-auto pt-3 border-t border-slate-800 flex gap-2">
                    {activeTab === 'library' ? (
                      ...
                      <>
                        ...
                        <button
                          onClick={(e) => handleOpenSettings(e, workflow)}
                          className="px-3 py-2 bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 text-xs rounded-xl font-semibold border border-blue-500/30 transition-all flex items-center justify-center gap-1.5"
                        >
                          <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {showSettingsModal && settingsWorkflow && (
          <div className="fixed inset-0 bg-black/70 backdrop-blur-md flex items-center justify-center z-50 p-4 overflow-y-auto">
            <div className="glass-card rounded-2xl premium-shadow max-w-3xl w-full my-8 overflow-hidden animate-scale-in">
              {/* Header */}
              <div className="relative p-8 border-b border-slate-800">
                ...
                <div className="relative flex items-start justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-slate-400">{settingsWorkflow.name}</p>
                    </div>
                  </div>
                  <button
                    onClick={() => {
              ...
              <div className="p-8 max-h-[60vh] overflow-y-auto">
                {(() => {
                  const fields = extractRequiredFields(settingsWorkflow);

                  if (fields.length === 0) {
                    return (
                      <div className="text-center py-12">
                        <div className="w-16 h-16 mx-auto mb-4 bg-gradient-to-br from-slate-700/40 to-slate-800/40 rounded-2xl flex items-center justify-center border border-slate-700">
                          <svg className="w-8 h-8 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                          </svg>
                        </div>
                        <h3 className="text-lg font-semibold text-white mb-2">No Configuration Needed</h3>
                        <p className="text-slate-400 text-sm">
                          This workflow doesn't require any additional configuration settings.
                        </p>
                      </div>
                    );
                  }
                    ...
                    <div className="space-y-6">
                      <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-4 mb-6">
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <p className="text-blue-400 font-medium text-sm mb-1">Configuration Required</p>
                            <p className="text-blue-300/80 text-xs">
                              This workflow requires configuration for {fields.length} field{fields.length > 1 ? 's' : ''}.
                              Enter the required credentials and settings to properly configure this automation.
                            </p>
                          </div>
                        </div>
                      </div>
                      ...
                      {fields.map((field) => (
                        <div key={field.key}>
                          ...
                          {field.type === 'textarea' ? (
                            <textarea
                              value={workflowConfig[field.key] || ''}
                            ...
                            <input
                              ...
                              placeholder={field.placeholder}
                              required={field.required}
                              className="w-full px-4 py-3 bg-slate-900/50 border border-slate-700 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:border-blue-500/50 focus:ring-2 focus:ring-blue-500/20 transition-all"
                            />
                          )}
                          <p className="text-xs text-slate-500 mt-1">
                            Node: {field.nodeName} ({field.nodeType})
                          </p>
                        </div>
                      ))}
                    </div>
              ...
              {/* Footer */}
              <div className="flex gap-3 p-6 border-t border-slate-800 bg-slate-950/80">
                <button
                  onClick={() => {
                    setShowSettingsModal(false);
                    setSettingsWorkflow(null);
                  ...
                  className="flex-1 px-6 py-3 bg-slate-800 hover:bg-slate-700 text-white font-medium rounded-xl transition-all"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="flex-1 px-6 py-3 bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 text-white font-semibold rounded-xl transition-all shadow-lg shadow-blue-500/30 hover:shadow-blue-500/50 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
                  {savingSettings ? (
                    <>
