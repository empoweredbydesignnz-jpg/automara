...
async function cloneWorkflowToFolder(workflowId, newName, folderId) {
  try {
    ...
    await axios.put(N8N_API_URL + '/workflows/' + result.id + '/tags', [folderId], {
      headers: { 'X-N8N-API-KEY': N8N_API_KEY, 'Content-Type': 'application/json' },
      timeout: 10000
    });
...
router.post('/workflows/:id/activate', async (req, res) => {
  ...
  try {
    ...
    await axios.patch(N8N_API_URL + '/workflows/' + clonedWorkflow.id, { active: true }, {
      headers: { 'X-N8N-API-KEY': N8N_API_KEY, 'Content-Type': 'application/json' },
      timeout: 10000
    });
...
router.post('/workflows/:id/deactivate', async (req, res) => {
  ...
  try {
    ...
    await axios.patch(N8N_API_URL + '/workflows/' + workflow.n8n_workflow_id, { active: false }, {
      headers: { 'X-N8N-API-KEY': N8N_API_KEY, 'Content-Type': 'application/json' },
      timeout: 10000
    });
    console.log('[DB] Updating workflow status');
    await pool.query('UPDATE workflows SET active = false, updated_at = NOW() WHERE id = $1', [id]);
    await pool.query('UPDATE workflow_activations SET deactivated_at = NOW(), status = $1 WHERE workflow_id = $2 AND status = $3', ['inactive', id, 'active']);
